...
impl SmartSolver {
    ...
    fn put_hallway(&mut self, pos: GridPos) -> Result<(), ()> {
        debug_assert_eq!(self.level[pos], SmartSolverCell::Unknown);
    ...
    fn fill_out_logical_values(&mut self) -> Result<(), ()> {
        ...
        while changed {
            ...
            for x in 0..self.level.width() {
                ...
                if self.col_missing_walls[x] == unknown_count {
                    for y in 0..self.level.height() {
                        let pos = (x, y).into();
                        if self.level[pos] == SmartSolverCell::Unknown {
                            self.put_wall(pos)?;
            ...
            for idx in 0..self.unsatisfied_monsters.len() {
                ...
                let mut sure_ways_out = self
                    .level
                    .iter_neighbors(pos)
                    .filter(|&n| {
                        self.level[n] != SmartSolverCell::Wall
                ...
                if let Some(sure_way_out) = sure_ways_out.pop() {
                    if self.level[sure_way_out] == SmartSolverCell::Unknown {
                        self.put_hallway(sure_way_out)?;
                        changed = true;
                    }
                    for n in self
                        .level
                        .iter_neighbors(sure_way_out)
            ...
            // Put walls to avoid wide hallways
            for y in 1..self.level.height() {
                for x in 1..self.level.width() {
                    if matches!(
                        (
                            self.level[(x - 1, y - 1).into()],
                            self.level[(x - 1, y).into()],
                            self.level[(x, y - 1).into()],
                            self.level[(x, y).into()],
                        ),
                        (
                            SmartSolverCell::Hallway,
                        ...
                        self.put_wall((x, y).into())?;
                        changed = true;
                    }
                }
            }

            // Fill out hallway where wall no longer possible
            for y in 0..self.level.height() {
                if self.row_missing_walls[y] > 0 {
                    continue;
                }
                for x in 0..self.level.width() {
                    if self.col_missing_walls[x] > 0 {
                        continue;
                    }
        ...
        for treasure_room_x in ((treasure.x as isize - 2).max(0) as usize)..=treasure.x {
            for treasure_room_y in ((treasure.y as isize - 2).max(0) as usize)..=treasure.y {
                ...
                for dx in 0..3 {
                    for dy in 0..3 {
                        ...
                        match self.level[pos] {
                            SmartSolverCell::Wall
                            | SmartSolverCell::Hallway
                            | SmartSolverCell::Monster => {
                                valid_inside = false;
                                break;
                            }
                            SmartSolverCell::Treasure | SmartSolverCell::Unknown => {}
                        }
                    }
                    if !valid_inside {
                        break;
                ...
                if perimiter.iter().any(|&pos| {
                    !matches!(
                        self.level[pos],
                        SmartSolverCell::Wall | SmartSolverCell::Unknown
                    )
                }) {
                    continue;
                }
                ...
                let potential_exits = perimiter
                    ...
                    .filter(|&p| {
                        p.x > 1
                            && p.y > 1
                            && p.x + 1 < self.level.width()
                            && p.y + 1 < self.level.height()
    ...
    pub fn all_solutions(&mut self) -> Vec<Level> {
        ...
        if let Some(unhandled_treasure) = self.unhandled_treasures.pop() {
            ...
            if let Some(next_pos) = self.next_pos {
                ...
                let mut solver = self.clone();
                if solver.put_hallway(next_pos).is_ok() {
                    solutions.extend(solver.all_solutions());
                }
            }
        }
